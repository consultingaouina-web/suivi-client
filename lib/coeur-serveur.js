...
if(require.main === module){
  const { SESSION_SECRET, MONGODB_URI, DB_MODE, PORT = 3000 } = process.env;
  if(!SESSION_SECRET){
    console.error("Variable d'environnement manquante. SESSION_SECRET est obligatoire (voir .env.example). Le serveur ne peut pas démarrer.");
    process.exit(1);
  }
  ...
  (async () => {
    try{
      let db;
      if(modeFichier){
        const { DbFichier } = require('./db-fichier');
        // dossier data/ à la racine du projet (un niveau au-dessus de lib/),
        // pour que l'utilisateur le retrouve facilement à côté de public/.
      ...
      const app = creerApp({ db, sessionSecret: SESSION_SECRET });
      app.listen(PORT, () => console.log(`Serveur démarré sur le port ${PORT} (${modeFichier ? 'stockage local JSON' : 'MongoDB'})`));
    }catch(err){
